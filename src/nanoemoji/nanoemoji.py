...
def write_preamble(nw):
    def module_rule(mod_name, arg_pattern, rspfile=None, rspfile_content=None):
        nw.rule(mod_name, f"{sys.executable} -m nanoemoji.{mod_name} {arg_pattern}", rspfile=rspfile, rspfile_content=rspfile_content)

    nw.comment("Generated by nanoemoji")
    nw.newline()

    nw.rule("picosvg", "picosvg $in > $out")
    module_rule("write_codepoints", "$$(<$out.rsp) > $out", rspfile="$out.rsp", rspfile_content="$in")

...
flags.DEFINE_bool(
    "gen_ninja", True, "Whether to regenerate build.ninja"
)
flags.DEFINE_bool(
    "exec_ninja", True, "Whether to run ninja."
)
...
flags.DEFINE_string("output_dir", "build/bin", "Where to put the final binary.")
flags.DEFINE_string(
    "output_file",
    None,
...
def write_preamble(nw):
    ...
    nw.rule('codepointmap', 'nanoemoji_write_codepoints $in > $out')
    nw.rule('write_fea', 'nanoemoji_write_fea $in > $out')
    nw.newline()


def write_picosvg_builds(nw, svg_files):
    for svg_file in svg_files:
        dest_picosvg_file = os.path.join('picosvg', os.path.basename(svg_file))
        nw.build(dest_picosvg_file, 'picosvg', rel_build(svg_file))
    nw.newline()


def write_codepointmap_build(nw, svg_files):
    dest_file = 'codepointmap.csv'
    nw.build(dest_file, 'codepointmap', [rel_build(f) for f in svg_files])
    nw.newline()


def write_fea_build(nw, svg_files):
